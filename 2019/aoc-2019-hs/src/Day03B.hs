...
getClosestIntersectionDistance w1 w2 = minimum distances
    where
        ...
        distances = map (\p -> distanceToPoint w1 p + distanceToPoint w2 p) intersects

distance :: Point -> Point -> Int
distance (x1, y1) (x2, y2) = abs (x1 - x2) + abs (y1 - y2)

distanceToPoint :: Wire -> Point -> Int
distanceToPoint wire p = go wire p 0
    where
        go :: Wire -> Point -> Int -> Int
        go (segment:segments) p dist =
            if segmentContainsPoint segment p
                then distance p (fst segment) + dist
                else segmentLength segment + go segments p dist
        segmentContainsPoint s p = p `elem` getPoints s

segmentLength :: Segment -> Int
segmentLength (p1, p2) = distance p1 p2
